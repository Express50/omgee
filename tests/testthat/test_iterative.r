...
test_that("runs with RHO=FALSE", {
  ...
  actual <- iterative_function(1, c(0.00000001, 0.00000001, 0.00000001), dat = dat, RHO=FALSE)
  expect_equal(unname(actual)[1:3], expected, tolerance = 0.01)
})

test_that("runs with RHO=TRUE", {
  expected <- c(0.29, 0.55, 0.16)
  actual <- iterative_function(10, c(0.00000001, 0.00000001, 0.00000001), dat = dat, RHO=TRUE)
  expect_equal(unname(actual)[1:3], expected, tolerance = 0.01)
})
